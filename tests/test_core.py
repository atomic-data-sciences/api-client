...
def test_core_get_ok(httpserver: HTTPServer):
    ...
    httpserver.expect_request("/test", headers={"X-API-KEY": "key_test"}, query_string={"param_foo": "param_bar"}).respond_with_json({"foo_sub": "bar_sub"})
    ...
    response = base_client._get(sub_url="test", params={"param_foo": "param_bar"}, deserialize=False)
    assert isinstance(response, bytes)

    response = base_client._get(sub_url="", base_override=httpserver.url_for("/test"), params={"param_foo": "param_bar"})
    assert response.get("foo_sub") == "bar_sub" # type: ignore
